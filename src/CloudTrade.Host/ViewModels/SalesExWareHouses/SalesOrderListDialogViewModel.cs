...
{
    public class SalesOrderListDialogViewModel:BindableBase
    {
        private readonly ISalesOrderService db;
        public SalesOrderListDialogViewModel(ISalesOrderService db)
        {
            this.db = db;
            InitData();
        }
            ...
            if (arg.Count == 2)
            {
                if (arg[0] is System.Windows.Window view)
                {
                    ...
                    if (arg[1] is IList item)
                    {
                        if (item.Count == 1)
                        {
                            if (item[0] is SalesOrder entity)
                            {
        ...
        public async void InitData(int PageSize = 10, string query = "", string StartTime = "", string EndTime = "")
        {
            try
            {
                ...
                SalesOrderList = new ObservableCollection<SalesOrderDto>(list);
            }
            catch (Exception ex)
            {
                Growl.Error(ex.Message);
