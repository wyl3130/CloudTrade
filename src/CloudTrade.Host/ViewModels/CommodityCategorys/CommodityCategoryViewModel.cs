...
{
    public class CommodityCategoryViewModel : BindableBase, INavigationAware
    {
        private readonly IEventAggregator eventAggregator;
        private readonly ICommodityCategoryService db;

        public CommodityCategoryViewModel(ICommodityCategoryService db, IEventAggregator eventAggregator)
        {
            this.db = db;
            ...
            get
            {
                return new DelegateCommand(async () =>
                {
                    try
                    {
                        ...
                        var view = new CommodityCategoryDialogView();
                        view.Owner = System.Windows.Application.Current.MainWindow;
                        view.WindowStartupLocation = WindowStartupLocation.CenterOwner;
                        var vm = new CommodityCategoryDialogViewModel(db);
                        vm.Title = "添加";
                        view.DataContext = vm;
                ...
                return new DelegateCommand<IList>(async (arg) =>
                {
                    try
                    {
                        ...
                        else
                        {
                            ...
                            {
                                AppData.ShowLoadingWindow();
                                foreach (CommodityCategory item in arg)
                                {
            ...
            get
            {
                return new DelegateCommand<IList>(async (arg) =>
                {
                    try
                    {
                        ...
                        else if (arg[0] is CommodityCategory entity)
                        {
                            eventAggregator.GetEvent<VisibilityChangeEvent>().Publish();
                            var view = new CommodityCategoryDialogView();
                            var vm = new CommodityCategoryDialogViewModel(db);
        ...
        public async void InitData(int PageSize=0,string query = "")
        {
            try
            {
                (var list, PageCount) = await db.CommodityCategoryQueryAsync(PageIndex, PageSize, query);
                CommodityCategoryList = new ObservableCollection<CommodityCategory>(list);
            }
            catch (Exception ex)
            {
                Growl.Error(ex.Message);
