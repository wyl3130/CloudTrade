...
{
    public class ModeInfoViewModel : BindableBase, INavigationAware
    {
        private readonly IModeInfoService db;
        private readonly IEventAggregator eventAggregator;
        public ModeInfoViewModel(IModeInfoService db, IEventAggregator eventAggregator)
        {
            this.db = db;
            InitData();
            ...
            get
            {
                return new DelegateCommand(async () =>
                {
                    try
                    {
                        ...
                        var view = new ModeInfoDialogView();
                        view.Owner = System.Windows.Application.Current.MainWindow;
                        view.WindowStartupLocation = WindowStartupLocation.CenterOwner;
                        var vm = new ModeInfoDialogViewModel(db);
                        vm.Title = "添加";
                        view.DataContext = vm;
                        ...
                        else
                        {
                            ...
                            if (dialog == MessageBoxResult.Yes)
                            {
                                ...
                                foreach (ModeInfo item in arg)
                                {
                                    ...
                                    //}
                                    await db.DeleteAsync<ModeInfo>(item);
            ...
            get
            {
                return new DelegateCommand<IList>(async (arg) =>
                {
                    try
                    {
                        ...
                        else if (arg[0] is ModeInfo entity)
                        {
                            eventAggregator.GetEvent<VisibilityChangeEvent>().Publish();
                            var view = new ModeInfoDialogView();
                            var vm = new ModeInfoDialogViewModel(db);
        ...
        public async void InitData(int PageSize=10,string query = "")
        {
            try
            {
                (var list, PageCount) = await db.ModeInfoQueryAsync(PageIndex,PageSize,query);
                ModeInfoList = new ObservableCollection<ModeInfo>(list);
            }
            catch (Exception ex)
            {
                Growl.Error(ex.Message);
