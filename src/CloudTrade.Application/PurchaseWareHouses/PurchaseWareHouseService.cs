...
{
    public class PurchaseWareHouseService : CommodityService, IPurchaseWareHouseService
    {
        ...
        public async Task<bool> PurchaseWareHouseDeleteAsync(PurchaseWareHouse entity)
        {
            try
            {
                await db.Ado.BeginTranAsync();
                ...
                var result = await db.Deleteable<PurchaseWareHouse>(entity).ExecuteCommandHasChangeAsync();
                if (result)
                {
                    // 删除 PurchaseWareHouseItem 记录时，先检查是否存在相关记录
                    var hasItems = await db.Queryable<PurchaseWareHouseItem>()
                                           .AnyAsync(x => x.PurchaseWareHouseId.Equals(entity.Id));
                    if (hasItems)
                    {
                        var resulti = await db.Deleteable<PurchaseWareHouseItem>(x => x.PurchaseWareHouseId.Equals(entity.Id)).ExecuteCommandHasChangeAsync();
                        if (!resulti)
                        {
                            // 如果子项删除失败，回滚事务
                            await db.Ado.RollbackTranAsync();
